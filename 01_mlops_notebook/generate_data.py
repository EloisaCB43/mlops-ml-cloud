...
class UserGenerator:  # clase para generar usuarios sintéticos
    def __init__(self, n_samples=1000, seed=42):   # Nuestro constructor, --init-- es el método que se ejecuta al crear una instancia de la clase, 
        self.n_samples = n_samples  # guardamos los valores de nuestros paramentros con self. para que se puedan usar en otros métodos de la clase
        self.seed = seed
    ...
    def generate_synthetic_users(self):
        ...
        """
        np.random.seed(self.seed)  # activa la semilla en Numpy
        random.seed(self.seed)  # activa la semilla en la librería random de Python
        ...
        for i in range(self.n_samples):
            ...
            # Perfil del usuario
            # los wieghts nos permiten asignar diferentes probabilidades a cada categoría, lo que hace que los datos sean más realistas
            # random.choices() nos ayuda a seleccionar una categoría de esas probabilidades
            # se hace [0] porque devuelve una lista y queremos el primer elemento
    ...
    def add_missing_data(self, df):
        ...
        for column, null_prob in null_config.items():
            if column in df_with_nulls.columns:
                ...
                null_mask = np.random.random(len(df_with_nulls)) < null_prob   # genera un array de v aleatorios entre 0 y 1, y compara cada valor con la probabilidad de nulo para esa columna, lo que da como resultado un array booleano donde True indica que se debe asignar un valor nulo
                
                # Aplicar valores nulos
                df_with_nulls.loc[null_mask, column] = np.nan # asigna NaN a las filas donde null_mask es True para la columna actual
